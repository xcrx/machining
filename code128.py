...
#courbB08.pil PIL Font file unencoded
courB08_pil = """
eJztl91rFkcUxp+Zt7vGFYzVtiJKICgYlLRWkaBBVGgDraFGCH5gsQp+QMBqabAVRYJYAlakCkoh
...
#courbB08.pbm font file unencoded
courB08_pbm = """
eJxNkntM01cUx8+P2/1apUAZEpECq4KRjKhF0E55FYEp4yG6mglz2Q8Q1BhERhhls/zKI+CID4wb
...
class Code128:
    ...
    CharSetC = {'00': 0, '01': 1, '02': 2, '03': 3, '04': 4, '05': 5, '06': 6, '07': 7,
        ...
        for c in range(len(code)):
            ...
            if (len(code[c:]) >= 4 and code[c:c+4].isdigit() and current_charset != self.CharSetC or
                ...
                if current_charset != self.CharSetC:
                    ...
                    if pos:
                        str_code += self.ValueEncodings[current_charset['Code C']]
                        total += pos * current_charset['Code C']
                    else:
                        str_code = self.ValueEncodings[self.CharSetC['START C']]
                        total = self.CharSetC['START C']
                ...
                if pos:
                    str_code += self.ValueEncodings[current_charset['Code B']]
                    total += pos * current_charset['Code B']
                else:
                    str_code = self.ValueEncodings[self.CharSetB['START B']]
                    total = self.CharSetB['START B']
                ...
                if pos:
                    str_code += self.ValueEncodings[current_charset['Code A']]
                    total += pos * current_charset['Code A']
                else:
                    str_code += self.ValueEncodings[self.CharSetA['START A']]
                    total = self.CharSetA['START A']
            ...
            total += pos * val
            str_code += self.ValueEncodings[val]
            pos += 1

        #Checksum
        checksum = total % 103

        str_code += self.ValueEncodings[checksum]

        #The stop character
        str_code += self.ValueEncodings[current_charset['STOP']]
    ...
    def get_image(self, value, height=50, path="tmp/"):
        """
        ...
        decode_font_file(courB08_pil, path+"courB08.pil")
        decode_font_file(courB08_pbm, path+"courB08.pbm")

        # Get the bar code list
        bits = self.make_code(value)
...
def decode_font_file(data, font):
    """
    ...
    if not exists(font):
        # Write font file
        open(font, "wb").write(decompress(decodestring(data)))